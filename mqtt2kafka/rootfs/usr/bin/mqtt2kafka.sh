...
JSON='[{"name": "mqtt", "url": "http://mqtt" },{"name":"wan","url":"http://wan"}]'

# OPTIONS
OPTIONS='{"log_level":"'${LOG_LEVEL}'","debug":'${DEBUG}',"services":'${JSON}',"period":'${MQTT2KAFKA_PERIOD}'}'
echo "${OPTIONS}" > ${TMPDIR}/${SERVICE_LABEL}.json

# make topic
TOPIC=$(curl -sSL -H 'Content-Type: application/json' -H "X-Auth-Token: ${MQTT2KAFKA_APIKEY}" "${MQTT2KAFKA_ADMIN_URL}/admin/topics" -d '{"name":"'${SERVICE_LABEL}'"}')
...
while true; do
  ...
  # wait for ..
  SECONDS=$((MQTT2KAFKA_PERIOD - $(($(date +%s) - DATE))))
