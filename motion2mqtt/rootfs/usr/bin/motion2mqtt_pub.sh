...
if [ -z "${MOTION_DEVICE:-}" ]; then echo "*** ERROR -- $0 $$ -- MOTION_DEVICE unspecified; exiting" &> /dev/stderr; exit 1; fi
...
if [ ! -z "${ARGS}" ]; then
  ...
  if [ "${DEBUG:-}" == 'true' ]; then echo "--- INFO -- $0 $$ -- publishing as ${MOTION_DEVICE} to ${MQTT_HOST} port ${MQTT_PORT} using arguments: ${ARGS}" &> /dev/stderr; fi
  mosquitto_pub -i "${MOTION_DEVICE}" -h "${MQTT_HOST}" -p "${MQTT_PORT}" ${ARGS} &> /dev/stderr
  exit $?
else
