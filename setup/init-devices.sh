...
if [ -n "${SETUP_ID}" ]; then
  ...
  if [ -z "${HORIZON_SETUP_URL}" ]; then
    echo "*** ERROR: cannot find URL for setup id ${SETUP_ID} in ${config}"
    exit 1
  fi
else
  echo "*** ERROR: no setup specified"
  exit 1
fi
...
if [ -n "${VENDOR_ID}" ]; then
  ...
  if [ -z "${VENDOR_TAG}" ]; then
    echo "*** ERROR: cannot find tag for vendor id ${VENDOR_ID} in configuration ${config}"
    exit 1
  fi
else
  echo "+++ WARN: no vendor specified for auto-discovery in ${config}"
  VENDOR_TAG=
fi
...
if [ -z "${NODES}" ]; then
  echo "+++ WARN: no nodes in configuration"
  NODE_COUNT=0
else
  if [ -n "${DEBUG}" ]; then echo "??? DEBUG: existing nodes:" $(echo ${NODES} | fmt -256); fi
  NODE_ARRAY=($(echo ${NODES}))
  NODE_COUNT=${#NODE_ARRAY[@]}
  echo "--- INFO: found ${NODE_COUNT} nodes by MAC in configuration"
fi

# find all devices from specified vendor
if [ -n "${VENDOR_TAG}" ]; then
  echo "--- INFO: searching for all from ${VENDOR_TAG}"
  vmacs=$(egrep "${VENDOR_TAG}" "${out}" | awk '{ print $3 }')
fi

if [ -n "${vmacs}" ]; then
  if [ -n "${DEBUG}" ]; then echo "??? DEBUG: ${VENDOR_TAG} devices found:" $(echo ${vmacs} | fmt -256); fi
  JSON=; for vmac in ${vmacs}; do 
    ...
    if [ -z "${FOUND}" ]; then
      ...
      if [ -n "${DEBUG}" ]; then echo echo "??? DEBUG: node ${vmac} in configuration ${config}"; fi
    fi
  done

  if [ -n "${JSON}" ]; then 
    JSON="${JSON}"']'
    if [ -n "${DEBUG}" ]; then echo "??? DEBUG: found" $(echo "${JSON}" | jq '.|length') "new ${VENDOR_TAG} nodes"; fi
    jq '.nodes+='"${JSON}" "${config}" > "$TMP/${config##*/}"; mv -f "$TMP/${config##*/}" "${config}"
    if [ -n "${DEBUG}" ]; then echo "??? DEBUG: updated configuration ${config}"; fi
  fi
else
  echo "--- INFO: no ${VENDOR_TAG} devices found"
fi

# update 
NODES=$(jq -r '.nodes[].mac' "${config}")
if [ -z "${NODES}" ]; then
  echo "+++ WARN: no nodes (MACs) in configuration"
  NODE_COUNT=0
else
  NODE_ARRAY=($(echo ${NODES}))
  NODE_COUNT=${#NODE_ARRAY[@]}
  echo "--- INFO: found ${NODE_COUNT} nodes (MACs) in configuration"
fi

echo "--- INFO: total of ${NODE_COUNT} nodes in configuration ${config}"
...
for MAC in ${MACS}; do 
  ...
  if [ -z "$id" ]; then
    ...
    echo "--- INFO: ${id}: found at MAC: $MAC; IP $client_ipaddr"
    # remove from known hosts
    if [ -s "${HOME}/.ssh/known_hosts" ]; then
      if [ -n "${DEBUG}" ]; then echo "??? DEBUG ${id}: removing $client_ipaddr from ${HOME}/.ssh/known_hosts"; fi
      egrep -v "${client_ipaddr}" "${HOME}/.ssh/known_hosts" > $TMP/known_hosts
      mv -f $TMP/known_hosts ${HOME}/.ssh/known_hosts
    fi
  fi

  if [ -n "${DEBUG}" ]; then echo "??? DEBUG ${id}: searching for configuration"; fi
  ...
  if [[ -z "${conf}" || "${conf}" == "null" ]]; then
    ...
    if [ -n "${DEBUG}" ]; then echo "??? DEBUG ${id}: found configuration $conf_id"; fi
  fi

  # find node state (cannot fail)
  node_state=$(jq '.nodes[]|select(.id=="'$id'")' "${config}")
  if [ -z "${node_state}" ] || [ "${node_state}" == 'null' ]; then
    echo "*** ERROR ${id}: found no existing node state; continuing"
    continue
  fi
  ...
  if [[ ${config_ssh} != "true" ]]; then
    ...
    cat "ssh-copy-id.tmpl" \
      ...
      > "$ssh_copy_id"
    if [ -n "${DEBUG}" ]; then echo "--- INFO: ${id}: SSH attempting copy-id ${ssh_copy_id} to ${client_ipaddr}"; fi
    success=$(expect -f "$ssh_copy_id" | egrep "success")
    if [[ "${success}" != "success"  ]]; then
      echo "+++ WARN: ${id} SSH failed copy-id; checking public key: $public_keyfile"
      ...
      if [ -z "${result}" ]; then
	echo "*** ERROR: ${id} SSH failed; ; consider reflashing; continuing"
	continue
      fi
    fi
  ...
  # sanity
  if [[ ${config_security} != "true" ]]; then
    echo "*** ERROR: ${id}: SECURITY failed; continuing"
    continue
  else
    # test access
  ...
  echo "--- INFO: ${id}: PATTERN configuration"
  # get pattern
  ...
  if [ -z "${result}" ]; then
    echo "*** ERROR: remote command $cmd returned zero results; continuing"
    continue
  else
  ...
  if [[ ${node_id} == ${ex_device} && $node_pattern == ${pt_org}/${pt_id} && ( $node_status == "configured" || $node_status == "configuring" ) ]]; then
    ...
    while [ -z "${result}" ] || [ $(echo "$result" | jq '.configstate.state=="unconfigured"') == false ]; do
      if [ -n "${DEBUG}" ]; then echo "??? DEBUG: ${id}: waiting on unregistration [10];" $(echo "$result" | jq -c '.'); fi
      sleep 10
      result=$(ssh -o "CheckHostIP no" -o "StrictHostKeyChecking no" -i "$private_keyfile" "$client_username"@"$client_ipaddr" "$cmd 2> /dev/null")
    done
    if [ -z "${result}" ]; then
      echo "*** ERROR: ${id}: command $cmd returned zero results; continuing"
      continue
    fi
    node_state=$(echo "$node_state" | jq '.node='"$result")
  fi
  ...
  while [ -z "${result}" ] || [ $(echo "$result" | jq '.configstate.state=="configured"') == false ]; do
    if [ -n "${DEBUG}" ]; then echo "??? DEBUG: ${id}: waiting on configuration [10]" $(echo "$result" | jq -c '.'); fi
    sleep 10
    result=$(ssh -o "CheckHostIP no" -o "StrictHostKeyChecking no" -i "$private_keyfile" "$client_username"@"$client_ipaddr" "$cmd 2> /dev/null")
  done
  if [ -z "${result}" ] || [ "${results}" == "null" ]; then
    echo "*** ERROR: ${id}: command $cmd failed; continuing"
    continue
  fi
  node_state=$(echo "$node_state" | jq '.node='"$result")
  ...
  while [ -z "${result}" ] || [ $(echo "$result" | jq '.==[]') == "true" ]; do
    if [ -n "${DEBUG}" ]; then echo "??? DEBUG: ${id}: waiting on agreement [10]" $(echo "$result" | jq -c '.'); fi
    sleep 10
    result=$(ssh -o "CheckHostIP no" -o "StrictHostKeyChecking no" -i "$private_keyfile" "$client_username"@"$client_ipaddr" "$cmd 2> /dev/null")
  done
