...
### Services
A _service_ is a packaging of **one** LINUX _container_ image with zero or more other services.  Composition with required services via _virtual private network_ (VPN) with each service accessed (e.g. RESTful API) by well-known host **name** [where does this come from? ED] and port number.  Required services are identified by `url` as well as its architecture and version; for example:
...
Services are published to the exchange through an intermediary container registry service, e.g. Docker Hub.  The container image in the registry is copied [are we sure? ED] into exchange when the service is published.
...
+ Patterns
  + cpu2msghub - send CPU and GPS data to Kafka 
  + sdr2msghub - send audio from FM radio broadcasts and send to Watson STT and NLU
+ Services
...
The [`examples`][open-horizon-examples-github] repository provides a breakdown into three subdirectories:
  ...
  + `tools/` - source code for `kafkacat`
...
The `tools/` and `cloud/` components are out-of-scope.  The `edge/` subdirectory contains subdirectories for documentation (`doc/`), as well as for the following:
  ...
  + `wiotp` - Watson IoT Platform (_deprecated_)
...
## `edge/msghub/cpu2msghub`
 ...
 This subdirectory contains the source code and build files for the `cpu2msghub` _service_.   The `cpu2msghub` _service_ requires two additional services form the `IBM` organization, nominally _cpu_ and _gps_,  identified by their sets of `url`,`arch`, and `version`.
...
1. Automate creation of `dev` environment (and elimination of `horizon/` artefact) 
...
1. `TAG` builds - optional environment variable or`TAG` file to distinguish build artefacts
1. Create script `fixpattern.sh` - automate pattern naming for publishing

### 1. Configuration JSON

The specification of version numbers and other dependencies for the build process were centralized into two JSON configuration files: `service.json` and `pattern.json`.  These files existed in the original, but were elevated to build artefacts and were templatized to handle issues of architecture and required services identification, notably the _tagging_ of the `url` and the `requiredServices[].url` in the `service.json` and `services[].servicesUrl` in the `pattern.json` (n.b. see _`mkdepend.sh`_).
...
Simplify development process through single Dockerfile using the JSON configuration in `build.json` to derive the `FROM` specification.  The `build.json` contains an array of **supported** architectures and the corresponding LINUX container **tag**, for example:
...
The static artefact of the `horizon/` directory was specific to the cpu2msghub service, including files with embedded environment variables which required evaluation to create necessary components in the build.  Nothing in the `horizon/` directory could not be generated using the `hzn dev service new` command.

### 7. and 8. and 9. `mkdepend.sh`, `userinput.json`,`checkvars.sh`

The template artefacts of `service.json` and `userinput.json` are processed to provide proper architectures and variables for use with `hzn dev service start` command, including checking variables against transient files with matching names, e.g. `MSGHUB_APIKEY`.  This file-name mechanism enables automation of the build process as avoids disclosue of _secrets_ using TravisCI.
...
To avoid utilization of the same pattern _name_ when building the software on varying branches in the repository, an additional `TAG` is introduced to append to all artefacts built in the repository.  The `fixpattern.sh` script appends the value, if and only if defined, to the pattern _name_ for both the hzn exchange pattern publish` command and derived, transient, `horizon/pattern.json` file.
